...
class UserController {
    ...
    signup = async (req, res, next) =>{
        try{
            ...
            await this.userService.signup( locationId, nickname, password, email, profileImage )
    ...
    login = async (req, res, next) =>{
        ...
        res.cookie('Authorization', `Bearer ${loginUser}`, {maxAge:3600});
        res.send({ token: `Bearer ${loginUser}` });
    }
    ...
    emailDup = async (req, res, next) =>{
        ...
        return res.status(400).json({
            ok: false, errorMessage:"이메일이 이미 존재합니다"
        })
        } else {
    ...
    nicknameDup = async (req, res, next) =>{
        ...
        if (nicknameDup){
        return res.status(400).json({
            ok: false, errorMessage:"닉네임이 이미 존재합니다"
        })
        } else {
