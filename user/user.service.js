...
class UserService {
    ...
    signup = async( locationId, nickname, password, email, profileImage )=>{
        ...
        return await this.userRepository.signup({locationId, nickname, password, email, profileImage, createdAt: Date.now()+'', updatedAt: Date.now()+''})
    }

    login = async(email, password)=>{
        const loginUser = await this.userRepository.userFindEmail(email);
        if( !loginUser || !(await bcript.compare(password, loginUser.get().password)) )
        throw new Error('아이디 혹은 비밀번호가 일치하지 않습니다')

        // 토큰 만들기
        return jwt.sign({userId : loginUser.userId}, 'secret_dang', {expiresIn: '1h'})
    }
